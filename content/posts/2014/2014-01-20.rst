...
- `Cinnamon`_. Fork [#]_ de *GNOME 3*. Su objetivo es permitirnos sacar
  provecho de las novedades de *GNOME 3* sin necesidad de sacrificar el tan
  apreciado interfaz de *GNOME 2*. Es el que elegí para mi padre: un entorno
...
- `Razor-qt`_. Entorno basado en `Qt`_, como *KDE*, que nos ofrece un entorno
  ...
  de *Archlinux*, parece que `en breve se fusionará`_ con `LXDE`_, dando lugar
  a *LXDE-Qt*.
...
Además, de ser ese es el caso, el `GRUB`_ [#]_ **necesita una**
...
Una vez tenemos el espacio libre, ya podemos crear la partición.
...
idéntico al de una tabla *MBR* [#]_.
...
En un entorno real, donde dispongamos del espacio suficiente:
...
- **Partición root (/)**. Con unos **18-20 GB (18432-2480 MB)** para la
  partición *root* debería estar bien.
...
Una vez realizadas las particiones convenientes, nos encontraríamos con algo
como esto:

.. figure:: {filename}/images/2014/01/2014-01-21_17.png
    :alt: Esquema final de particionado de los discos

    Si alguno de nuestros discos es SSD [#]_, marcaremos la casilla
    correspondiente.
...
Un vistazo rápido de confirmación a los cambios que vamos a hacer en los
discos y pasamos a los...
...
proceso de instalación en sí, en el que *Antergos* se copia en el disco duro.
...
extraeremos el medio una vez inicie.
...
1. Una memoria *USB*, formateada en
   *FAT16/FAT32 (opción recomendada)/NTFS (inestable)*, a partir de  *1 GB* de
...
El proceso de creación es , tanto en *Windows* como en **Linux**, muy sencillo:

1. Ejecutamos la aplicación. En **Linux** hemos de hacerlo con permisos de
   administrador:
...
4. Pulsamos *Write*.
5. Si la memoria se encontraba montada, nos pregunta sí queremos desmontarla
...
.. [#] Apuntes basados en `un boletín de servicio técnico`_ de *Toshiba*. **Hay que tener mucho cuidado con UEFI**. Busca documentación específica de tu fabricante.
.. [#] **Sistema Operativo**.
...
.. [#] Partición necesaria para arrancar en sistemas basados en *BIOS* donde el dispositivo desde el que arranca el *SO* tiene una tabla *GPT*.
